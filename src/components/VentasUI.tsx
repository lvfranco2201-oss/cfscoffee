  ...
  return (
    <div className="animate-in">
      ...
      <div className="grid-cols-6" style={{ marginBottom: '1.75rem' }}>
        {[
          { icon: <DollarSign size={18}/>, col: 'var(--cfs-gold)',  bg: 'var(--cfs-gold-dim)',        label: 'Ventas Netas Hoy',  val: fmt(data.kpisHoy.netSales),   sub: `Bruto: ${fmt(data.kpisHoy.grossSales)}`,           wow: data.wowSales },
          { icon: <Users size={18}/>,      col: 'var(--success)',   bg: 'rgba(46,202,127,0.12)',      label: 'Clientes Hoy',      val: data.kpisHoy.guests.toLocaleString(),  sub: `$/visita: ${fmt(avgPerGuest)}`,             wow: data.wowGuests },
          { icon: <ShoppingCart size={18}/>, col: 'var(--info)',    bg: 'rgba(79,172,254,0.12)',      label: 'Órdenes Cerradas',  val: data.kpisHoy.orders.toLocaleString(),  sub: `Ticket: ${fmt(avgTicket)}` },
          { icon: <Percent size={18}/>,    col: discPct > 8 ? 'var(--warning)' : 'var(--success)', bg: discPct > 8 ? 'rgba(245,158,11,0.12)' : 'rgba(46,202,127,0.12)', label: 'Descuentos',   val: fmt(data.kpisHoy.discounts), sub: `${discPct.toFixed(1)}% del bruto` },
          { icon: <AlertTriangle size={18}/>, col: voidPct > 2 ? 'var(--danger)' : 'var(--text-muted)', bg: 'rgba(239,68,68,0.08)', label: 'Voids + Refunds', val: fmt(data.kpisHoy.voids + data.kpisHoy.refunds), sub: `${voidPct.toFixed(2)}% del bruto` },
          { icon: <Award size={18}/>,      col: 'var(--cfs-gold)',  bg: 'var(--cfs-gold-dim)',        label: 'Mejor Día (90d)',    val: fmtK(best90),                 sub: data.topDias[0]?.date ? new Date(data.topDias[0].date + 'T12:00:00').toLocaleDateString('es-ES', { month: 'short', day: 'numeric' }) : '—' },
        ].map((c, i) => (
